package org.think2framework.weixin.exception;

/**
 * Created by zhoubin on 16/6/1. 微信错误异常
 */
public class WeixinErrorException extends RuntimeException {

	public static final String ERROR_CODE_SUCCESS = "0"; // 请求成功

	private String errCode; // 错误编号

	public WeixinErrorException() {
		super();
	}

	public WeixinErrorException(String errCode) {
		super(getMessage(errCode));
		this.errCode = errCode;
	}

	public WeixinErrorException(String errCode, Throwable cause) {
		super(getMessage(errCode), cause);
		this.errCode = errCode;
	}

	protected WeixinErrorException(String errCode, Throwable cause, boolean enableSuppression,
			boolean writableStackTrace) {
		super(getMessage(errCode), cause, enableSuppression, writableStackTrace);
		this.errCode = errCode;
	}
	...
	private static String getMessage(String code) {
		switch (code) {
			...
			case "41009" :
				return "缺少openid";
			case "42001" :
				return "access_token超时，请检查access_token的有效期";
			case "42002" :
			...
			case "48005" :
				return "api禁止删除被自动回复和自定义菜单引用的素材";
			case "48006" :
				return "api禁止清零调用次数，因为清零次数达到上限";
			case "50001" :
				return "用户未授权该api";
			case "50002" :
